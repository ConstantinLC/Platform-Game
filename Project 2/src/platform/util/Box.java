...
public final class Box {
	...
	/**
	 * Scales a given box in a certain direction
	 * @param scale
	 * @param dir
	 * @return
	 */
	public Box scale(double scale, String dir){
		if(scale == 1){
			return new Box(min, max);
		}
		if(dir.equals("Y"))
			return new Box(min, new Vector(max.getX(), min.getY()+(max.getY()-min.getY())*scale));
		if(dir.equals("X"))
			return new Box(min, new Vector( min.getX()+(max.getX()-min.getX())*scale, max.getY()));
		return null;
	}
