...
public class Boss extends Actor implements Living{
	...
	private double maxHealth = 12;

	public Boss(Vector initial, double radius, Actor target){
	...
	public void update(Input input){
		...
		if (time > 2*Math.PI && time < 4*Math.PI){
			if (!hasShot){
				Fireball f = new Fireball(getBox().getCenter(), target.getPosition().sub(getBox().getCenter()), this, 1.5);
				getWorld().register(f);
				hasShot = true;
			}
		}
