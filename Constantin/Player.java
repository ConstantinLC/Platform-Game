...
public class Player extends Actor implements Living{
	...
	private boolean fireball = true;
	private boolean doubleJump = true;
	private boolean bombs = true;
	...
	public void update(Input input) {
		...
		if(!teleporting){
			...
			if (input.getKeyboardButton(KeyEvent.VK_SPACE).isPressed() && fireball){
				getWorld().register(new Fireball(position, velocity.getX() >= 0 ? new Vector(5, velocity.getY()+3) : new Vector(-5, velocity.getY()+3), this, 3));
			}
			...
			if (input.getKeyboardButton(KeyEvent.VK_C).isPressed() && bombs) {
				...
				Bomb b = new Bomb(position, v, this, 0.2, 0.5);
				getWorld().register(b);
			}
		}
